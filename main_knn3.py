...
for i in range(t1):
    for j in range(n1):
        a.append(["Legitimate", compute_cosine_similarity(test_legitimate_tfidf[i], train_legitimate_tfidf[j])])
        b.append(["Spam", compute_cosine_similarity(test_legitimate_tfidf[i], train_spam_tfidf[j])])

    ab = a + b

    ab.sort(key=operator.itemgetter(1), reverse=True)

    tmp = []

    for z in range(knn_k):
        tmp.append(ab[z][0])


    if tmp.count("Legitimate") > tmp.count("Spam"):
...
for i in range(t1):
    for j in range(n1):
        b.append(["Spam", compute_cosine_similarity(test_spam_tfidf[i], train_spam_tfidf[j])])
        a.append(["Legitimate", compute_cosine_similarity(test_spam_tfidf[i], train_legitimate_tfidf[j])])
